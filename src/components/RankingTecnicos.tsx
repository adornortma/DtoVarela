    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '24px 32px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span style={{ fontSize: '11px', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '1px' }}>ANÁLISIS DE CALIDAD ({month})</span>
            <h3 style={{ fontSize: '20px', fontWeight: '950', color: '#0f172a', letterSpacing: '-0.5px', marginTop: '2px' }}>{tech.nombre}</h3>
          </div>
          <button onClick={onClose} style={{ border: 'none', background: '#f1f5f9', padding: '8px', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ padding: '32px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          ) : !data || (data.resueltas.total === 0 && data.noResueltas.total === 0) ? (
             <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', fontWeight: '700' }}>
               No se registraron actuaciones detalladas para este técnico en {month}.
             </div>
          ) : (
             <>
               {/* Resueltas */}
               <div>
                 <h4 style={{ fontSize: '13px', fontWeight: '900', color: '#059669', marginBottom: '14px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                   Actuaciones Resueltas ({data.resueltas.total})
                 </h4>
                 {data.resueltas.sorted.length === 0 ? (
                   <p style={{ fontSize: '13px', color: '#94a3b8' }}>Sin datos.</p>
                 ) : (
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                     {data.resueltas.sorted.slice(0, 5).map((item, idx) => (
                       <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                         ...
                         <div style={{ width: '100%', height: '6px', backgroundColor: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' }}>
                           <div style={{ width: `${(item.count / data.resueltas.max) * 100}%`, height: '100%', backgroundColor: '#10b981', borderRadius: '3px' }} />
                         </div>
                       </div>
                     ))}
                   </div>
               ...
               {/* No Resueltas */}
               <div>
                 <h4 style={{ fontSize: '13px', fontWeight: '900', color: '#e11d48', marginBottom: '14px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                   Actuaciones No Resueltas ({data.noResueltas.total})
                 </h4>
                 {data.noResueltas.sorted.length === 0 ? (
                   <p style={{ fontSize: '13px', color: '#94a3b8' }}>Sin datos.</p>
                 ) : (
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                     {data.noResueltas.sorted.slice(0, 5).map((item, idx) => (
                       <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                         <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: '700' }}>
                           <span style={{ color: '#334155' }}>{item.text}</span>
                           <span style={{ color: '#0f172a' }}>{item.count}</span>
                         ...
                         <div style={{ width: '100%', height: '6px', backgroundColor: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' }}>
                           <div style={{ width: `${(item.count / data.noResueltas.max) * 100}%`, height: '100%', backgroundColor: '#f43f5e', borderRadius: '3px' }} />
                         </div>
                       </div>
                     ))}
                   </div>
...
export default function RankingTecnicos({ districtSlug = 'varela' }: { districtSlug?: string }) {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
              ...
              <h2 style={{ fontSize: '28px', fontWeight: '950', letterSpacing: '-1px' }}>Ranking de Técnicos - {districtName}</h2>
            </div>

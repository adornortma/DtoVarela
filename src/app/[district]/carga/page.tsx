...
export default function CargaDistritoPage() {
  ...
  useEffect(() => {
    const fetchDistrictData = async () => {
      ...
      if (data) {
        ...
        setSelectedDistrictSlug(data.slug);
        setDistrictName(data.nombre);
        fetchCells(data.id);
      }
    };
    fetchDistrictData();
  ...
  const handleCheckDuplicates = async () => {
    ...
    if (!selectedDistrictId) {
      setSaveStatus({ type: 'error', msg: 'Distrito no cargado.' });
      return;
    }
      ...
      <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
            ...
            <h1 style={{ fontSize: '32px', fontWeight: '950', letterSpacing: '-1px' }}>Módulo de Carga de Datos</h1>
          </div>
          <p style={{ fontSize: '14px', fontWeight: '700', color: '#64748b' }}>
            Distrito {districtName} — Pega directamente el texto o tablas copiadas de navegadores o Excel.
          </p>
        </div>
      </div>
          ...
          <div style={{
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: '800' }}>
                ...
                <span style={{ color: '#1e293b' }}>{districtName}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: '800' }}>
